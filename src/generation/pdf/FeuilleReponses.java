...
public class FeuilleReponses extends CopiePdfImpl {

	public FeuilleReponses(Copie c) {
		super(c);
		...
		for (Question q : questions) {
			...
			try (PDPageContentStream pdPageContentStream = new PDPageContentStream(this.pdDocument,
				...
				if (q instanceof QuestionBoite) {
					heightOffset = this.generateOpenQ((QuestionBoite) q, qIndex, this.pdDocument,
							this.pdDocument.getPage(pageIndex), widthOffset, heightOffset, pageIndex);
				} else {
					heightOffset = this.generateQCM(q, qIndex, this.pdDocument, this.pdDocument.getPage(pageIndex),
							widthOffset, heightOffset, config, pageIndex);
				}
	...
	public float generateQCM(Question q, int qIndex, PDDocumentID pdDocument, PDPage curPage, float widthOffset,
		...
		try (PDPageContentStream pdPageContentStream = new PDPageContentStream(pdDocument, curPage,
			...
			for (Reponse r : reponses) {
				...
				r.setNumPage(numPage);
				pdPageContentStream.addRect(widthOffset + 20, this.height - heightOffset - 16,
						CopiePdfImpl.ID_CARRE_TAILLE, CopiePdfImpl.ID_CARRE_TAILLE);
	...
	public void generateHeader(Config config) {
		try {
			...
			PDPageID page = this.pdDocument.getPage(0);
			...
			super.generateNumEtudArea(config);
